...
const SignUp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await signUp(formData.name, formData.email, formData.password, formData.newsletter);
      alert(language === 'ar' ? 'تم إنشاء الحساب بنجاح' : 'Account created successfully');
    } catch (error) {
      alert(language === 'ar' ? 'حدث خطأ أثناء إنشاء الحساب' : 'Error creating account');
    }
  };
              ...
              <div className="space-y-4">
                <div className="flex items-start space-x-2">
                  ...
                  <Label htmlFor="terms" className="text-sm leading-relaxed">
                    ...
                    <Link to="/terms" className="text-primary hover:text-primary-hover">{language === 'ar' ? 'الشروط والأحكام' : 'Terms'}</Link>
                    {language === 'ar' ? ' و ' : ' and '}
                    <Link to="/privacy" className="text-primary hover:text-primary-hover">{language === 'ar' ? 'سياسة الخصوصية' : 'Privacy Policy'}</Link>
                  </Label>
                </div>
                <div className="flex items-start space-x-2">
                  <Checkbox
                  ...
                  />
                  <Label htmlFor="newsletter" className="text-sm">
                    {language === 'ar' ? 'أريد الحصول على العروض عبر البريد' : 'I want to receive offers via email'}
                  </Label>
                </div>
              </div>
